...
class RLExperimentManager:
    ...
    def build_model_hyperparameters(self, config):
        ...
        self.model_params["learning_rate"] = self.linear_schedule(self.model_params["learning_rate"])
        policy_kwargs = {}
        ...
        if self.algorithm == "ppo":
            policy_kwargs["net_arch"]["pi"] = [config["net_size"]]*2
            policy_kwargs["net_arch"]["vf"] = [config["net_size"]]*2
            del self.model_params["vf"]
            del self.model_params["net_size"]

        elif self.algorithm == "sac":
            policy_kwargs["net_arch"]["pi"] = [config["net_size"]]*2
            policy_kwargs["net_arch"]["qf"] = [config["net_size"]]*2
            del self.model_params["net_size"]
            ...
            del self.model_params["action_noise_type"],self.model_params["action_sigma"]
    ...
    def hyperparameter_tuning(self):
        ...
        if continue_sweep:
            wandb.agent("puk5fznz", project="dwarf-env", function=self.run_single_sweep, count=50)
        else:
            sweep_id = wandb.sweep(sweep=sweep_config, project=self.project)
            wandb.agent(sweep_id, function=self.run_single_sweep, count=50)
